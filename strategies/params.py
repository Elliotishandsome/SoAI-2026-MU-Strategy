...
TRADE_SYMBOLS = ["MU"]                 # 可交易標的（v2.8：純 MU）
...
# 槓桿倍數：1.0 = 無槓桿（僅可用資金）；2.0 = 最多買到 200% 總資金
# ⚠️ 回測實證：2x 槓桿使每筆倉位頂到 $2M 全倉 → 資金一次用盡 →
#    後續訊號無法下單（49 筆→3 筆），單筆大虧直接撞熔斷（-$15K）。
#    1x 已達 +$119K（Sharpe 1.25）。建議保持 1.0，槓桿參數僅供實驗。
LEVERAGE = 1.0
