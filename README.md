...
| 槓桿 | 買入 | 平均槓桿 | 勝率 | 日內淨損益 | Sharpe | 回撤 |
|------|------|---------|------|-----------|--------|------|
| **1x（預設）** | 49 | 0.95x | 68.7% | **+$119,069** | **1.25** | -1.4% |
| 2x | 3 | 1.99x | 0% | -$15,015 | -5.84 | -1.5% |

**結論**：倉位模型改善（equity 基準）使 1x 從 +$63K → **+$119K**（Sharpe
0.60 → 1.25）。**2x 槓桿反而災難**：每筆頂到 $2M 全倉 → 資金一次用盡 →
訊號剩 3 筆 → 單筆大虧撞熔斷。**強烈建議保持 LEVERAGE=1.0**（參數保留供實驗）。
